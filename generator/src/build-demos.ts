...
function baseSpec(kind: LayoutKind): SiteSpec {
  ...
  const NAMES: Record<LayoutKind, { name: string; tagline: string; head: string; sub: string }> = {
    ...
    bauunternehmen: { name: 'Steiner Bau GmbH', tagline: 'Hochbau · Tiefbau · Generalunternehmer seit 1962.',
                   head: 'Wir bauen, was bleibt.', sub: 'Vom Konzept bis zur schlüsselfertigen Übergabe — alle Gewerke aus einer Hand, koordiniert von einer Bauleitung, die wirklich am Bau steht.' },
